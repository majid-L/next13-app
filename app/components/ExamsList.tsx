...
const ExamsList = ({isLoading, exams, loggedInUser, setConfirmationMsg, setExams}: ExamsListProps) => {
...
const handleDelete = (id: number) => {
    ...
    .then(res => {
      if (res === 1) {
        ...
        setExams((prev: Exam[]) => prev.filter(exam => {
            if (exam.id != id) {
                return exam;
            }
        }))
        ...
        <div className="max-w-md mx-auto sm:px-6 lg:px-8">
             <div className="bg-white border-1 border-gray-700 shadow-lg shadow-pink-300/40 overflow-hidden rounded-lg">
                <div className="p-6 pb-4 text-gray-900 text-left">
                    ...
                    <p className="pb-2 border-b-2 border-t-indigo-500">{exam.description} is taking place in <span className="flex"><MapIcon/>{exam.locationName}, on</span><span className="flex"><CalendarIcon/>{exam.date.slice(0, 10)}, at</span><span className="flex"><TimeIcon/>{exam.date.slice(10)}.</span></p>
                    
                    <Link href={`/exams/${exam.id}`} className="block mt-2 text-blue-600 hover:underline hover:text-red-500">View exam page</Link>
                    {pathname === '/exams' && <Link href={`/candidates/${exam.candidateId}`} className="block mt-0.5 text-blue-600 hover:underline hover:text-red-500 overflow-ellipsis truncate">View {exam.candidateName}&apos;s exams</Link>}
