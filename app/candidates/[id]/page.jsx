...
<main className="pb-20 mx-auto w-11/12 sm:w-5/6">
  <h1 className="text-center text-stone-100 font-bold text-4xl md:text-5xl mt-12 mx-4 text-shadow-sm">{userIsAdmin(loggedInUser) ? `Viewing exam sessions for ${!isLoading ? exams[0]?.candidateName : '...'}` : 'My exams'}</h1>
  ...
  {exams.length > 0 ? <ExamsList exams={exams} setExams={setExams} loggedInUser={loggedInUser} isLoading={isLoading} setConfirmationMsg={setConfirmationMsg}/> 
